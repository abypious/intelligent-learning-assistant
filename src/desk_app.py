...
# -----------------------------
con=pymysql.connect(host='localhost',port=3306,user='root',password='',db='ila')
cmd=con.cursor()
# lb = tk.Listbox(root)
# lb.pack()
flag=False
...
def ffplay():
    ...
    #     file = lb.curselection()[0]
        ...
        print("SELECT v_f_id,max(ratio),id FROM video_frame WHERE std_id=" + str(stu) + " AND ratio > 0.5")
        s = cmd.fetchone()
        print(s)
        cmd.execute("SELECT mat_type FROM questions WHERE id=" + str(s[2]))
        r=cmd.fetchone()
        print(r)
        lr = Label(root, text=r[0])
    ...
    while(flag1):
     ...
     while(flag):
        ...
        for (x,y,w,h) in faces:
            ...
            print (emotion)
            time.sleep(5)
        cv2.imwrite("static/pic/"+str(i)+".png",img)
        print("okkkkkkkkkkkkkkkkkkkkkkkkk111111111111111111111111")
            # if cv2.waitKey(1):
        # cv2.imshow('img', img)
